...
public class R$primitive$max
    extends BuiltinFunction
{


    public R$primitive$max() {
        super("max");
    }
    ...
    public SEXP apply(Context context, Environment environment, FunctionCall call, PairList args) {
        try {
            ...
            if ((varArgs.length()> 0)&&((AbstractSEXP) varArgList.getElementAsSEXP(0)).isObject()) {
                SEXP genericResult = S3 .tryDispatchSummaryFromPrimitive(context, environment, call, "max", varArgList, flag0);
                if (genericResult!= null) {
                    return genericResult;
                }
            }
            return Summary.max(varArgList, flag0);
        } catch (ArgumentException e) {
            throw new EvalException(context, "Invalid argument: %s. Expected:\n\tmax(..., logical(1))", e.getMessage());
    ...
    public SEXP apply(Context context, Environment environment, FunctionCall call, String[] argNames, SEXP[] args) {
        return R$primitive$max.doApply(context, environment, call, argNames, args);
    }
