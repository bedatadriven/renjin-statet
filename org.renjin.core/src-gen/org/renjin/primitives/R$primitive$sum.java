...
public class R$primitive$sum
    extends BuiltinFunction
{


    public R$primitive$sum() {
        super("sum");
    }
    ...
    public SEXP apply(Context context, Environment environment, FunctionCall call, PairList args) {
        try {
            ...
            if ((varArgs.length()> 0)&&((AbstractSEXP) varArgList.getElementAsSEXP(0)).isObject()) {
                SEXP genericResult = S3 .tryDispatchSummaryFromPrimitive(context, environment, call, "sum", varArgList, flag0);
                if (genericResult!= null) {
                    return genericResult;
                }
            }
            return Summary.sum(varArgList, flag0);
        } catch (ArgumentException e) {
            throw new EvalException(context, "Invalid argument: %s. Expected:\n\tsum(..., logical(1))", e.getMessage());
    ...
    public SEXP apply(Context context, Environment environment, FunctionCall call, String[] argNames, SEXP[] args) {
        return R$primitive$sum.doApply(context, environment, call, argNames, args);
    }
