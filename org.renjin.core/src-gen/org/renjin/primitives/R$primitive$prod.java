...
public class R$primitive$prod
    extends BuiltinFunction
{


    public R$primitive$prod() {
        super("prod");
    }
    ...
    public SEXP apply(Context context, Environment environment, FunctionCall call, PairList args) {
        try {
            ...
            if ((varArgs.length()> 0)&&((AbstractSEXP) varArgList.getElementAsSEXP(0)).isObject()) {
                SEXP genericResult = S3 .tryDispatchSummaryFromPrimitive(context, environment, call, "prod", varArgList, flag0);
                if (genericResult!= null) {
                    return genericResult;
                }
            }
            return WrapperRuntime.wrapResult(Summary.prod(varArgList, flag0));
        } catch (ArgumentException e) {
            throw new EvalException(context, "Invalid argument: %s. Expected:\n\tprod(..., logical(1))", e.getMessage());
    ...
    public SEXP apply(Context context, Environment environment, FunctionCall call, String[] argNames, SEXP[] args) {
        return R$primitive$prod.doApply(context, environment, call, argNames, args);
    }
