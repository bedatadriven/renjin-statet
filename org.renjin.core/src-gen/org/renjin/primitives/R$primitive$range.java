...
public class R$primitive$range
    extends BuiltinFunction
{


    public R$primitive$range() {
        super("range");
    }
    ...
    public SEXP apply(Context context, Environment environment, FunctionCall call, PairList args) {
        try {
            ...
            if ((varArgs.length()> 0)&&((AbstractSEXP) varArgList.getElementAsSEXP(0)).isObject()) {
                SEXP genericResult = S3 .tryDispatchSummaryFromPrimitive(context, environment, call, "range", varArgList, flag0);
                if (genericResult!= null) {
                    return genericResult;
                }
            }
            return Summary.range(varArgList, flag0);
        } catch (ArgumentException e) {
            throw new EvalException(context, "Invalid argument: %s. Expected:\n\trange(..., logical(1))", e.getMessage());
    ...
    public SEXP apply(Context context, Environment environment, FunctionCall call, String[] argNames, SEXP[] args) {
        return R$primitive$range.doApply(context, environment, call, argNames, args);
    }
