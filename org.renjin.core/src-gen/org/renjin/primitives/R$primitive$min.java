...
public class R$primitive$min
    extends BuiltinFunction
{


    public R$primitive$min() {
        super("min");
    }
    ...
    public SEXP apply(Context context, Environment environment, FunctionCall call, PairList args) {
        try {
            ...
            if ((varArgs.length()> 0)&&((AbstractSEXP) varArgList.getElementAsSEXP(0)).isObject()) {
                SEXP genericResult = S3 .tryDispatchSummaryFromPrimitive(context, environment, call, "min", varArgList, flag0);
                if (genericResult!= null) {
                    return genericResult;
                }
            }
            return Summary.min(varArgList, flag0);
        } catch (ArgumentException e) {
            throw new EvalException(context, "Invalid argument: %s. Expected:\n\tmin(..., logical(1))", e.getMessage());
    ...
    public SEXP apply(Context context, Environment environment, FunctionCall call, String[] argNames, SEXP[] args) {
        return R$primitive$min.doApply(context, environment, call, argNames, args);
    }
